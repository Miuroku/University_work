...
class PickleParser:
    ...
    def get_funcs(obj, visited):
        for i in obj.__globals__:
            ...
            if inspect.isfunction(attr) and attr.__name__ not in visited:
                ...
                visited = PickleParser.get_funcs(attr, visited)
        return visited
    ...
    def dict_to_func(obj):
        func = types.FunctionType(
            globals=PickleParser.parse(obj["__globals__"]),
            code=PickleParser.parse(obj["__code__"]),
            name=obj["__name__"],
            argdefs=obj["__defaults__"],
        )

        funcs = PickleParser.get_funcs(func, {func.__name__: func})
        PickleParser.set_funcs(funcs)
        ...
        if len(cls.__bases__) != 0:
            for i in cls.__bases__:
                if i.__name__ != "object":
                    depends.append(PickleParser.class_to_dict(i))
        args = {}
        cls_dict = dict(cls.__dict__)
        if len(cls_dict) != 0:
            for i in cls_dict:
                if inspect.isclass(cls_dict[i]):
                    args[i] = PickleParser.class_to_dict(cls_dict[i])
                elif inspect.isfunction(cls_dict[i]):
                    if cls_dict[i] not in f_found:
                        args[i] = PickleParser.function_to_dict(cls_dict[i])
                elif isinstance(cls_dict[i], staticmethod):
                    if cls_dict[i].__func__ not in f_found:
                        args[i] = PickleParser.staticmethod_to_dict(cls_dict[i])
                elif isinstance(cls_dict[i], classmethod):
                    if cls_dict[i].__func__ not in f_found:
                        args[i] = PickleParser.classmethod_to_dict(cls_dict[i])
                elif inspect.ismodule(cls_dict[i]):
                    args[i] = PickleParser.module_to_dict(cls_dict[i])
                elif PickleParser.is_cls_instance(cls_dict[i]):
                    args[i] = PickleParser.instance_to_dict(cls_dict[i])
    ...
    def instance_to_dict(obj):
        return {
            "--instance_type--": {
                "class": PickleParser.class_to_dict(obj.__class__),
                "vars": obj.__dict__,
            }
        }
    ...
    def gather_gls(obj, obj_code):
        ...
        for i in obj_code.co_names:
            try:
                if inspect.isclass(obj.__globals__[i]):
                    gls[i] = PickleParser.class_to_dict(obj.__globals__[i])
                elif inspect.isfunction(obj.__globals__[i]):
                    if obj.__globals__[i] not in f_found:
                        gls[i] = PickleParser.function_to_dict(obj.__globals__[i])
                elif isinstance(obj.__globals__[i], staticmethod):
                    if obj.__globals__[i].__func__ not in f_found:
                        gls[i] = PickleParser.staticmethod_to_dict(obj.__globals__[i])
                elif isinstance(obj.__globals__[i], classmethod):
                    if obj.__globals__[i].__func__ not in f_found:
                        gls[i] = PickleParser.classmethod_to_dict(obj.__globals__[i])
                elif inspect.ismodule(obj.__globals__[i]):
                    gls[i] = PickleParser.module_to_dict(obj.__globals__[i])
                elif PickleParser.is_cls_instance(obj.__globals__[i]):
                    gls[i] = PickleParser.instance_to_dict(obj.__globals__[i])
                elif isinstance(
        ...
        for i in obj_code.co_consts:
            if isinstance(i, types.CodeType):
                gls.update(PickleParser.gather_gls(obj, i))
        return gls

    @staticmethod
    def staticmethod_to_dict(obj):
        return {"--static_method_type--": PickleParser.function_to_dict(obj.__func__)}

    @staticmethod
    def classmethod_to_dict(obj):
        return {"--class_method_type--": PickleParser.function_to_dict(obj.__func__)}

    @staticmethod
    def function_to_dict(obj):
        gls = PickleParser.gather_gls(obj, obj.__code__)

        return {
            "--function_type--": {
                "__globals__": gls,
                "__name__": obj.__name__,
                "__code__": PickleParser.code_to_dict(obj.__code__),
    ...
    def code_to_dict(obj):
        return {
            "--code_type--": {
                ...
                "co_code": obj.co_code,
                "co_consts": PickleParser.convert(obj.co_consts),
    ...
    def convert(obj):
        if isinstance(obj, (str, int, float, bool, frozenset)) or obj is None:
            ...
            for i in range(len(obj)):
                obj[i] = PickleParser.convert(obj[i])
            return obj
            ...
            res = {}
            for i in obj:
                res[i] = PickleParser.convert(obj[i])
            return res
        elif inspect.isfunction(obj):
            res = PickleParser.function_to_dict(obj)
            return res
        elif isinstance(obj, staticmethod):
            res = PickleParser.staticmethod_to_dict(obj)
            return res
        elif isinstance(obj, classmethod):
            res = PickleParser.classmethod_to_dict(obj)
            return res
        elif inspect.ismodule(obj):
            return PickleParser.module_to_dict(obj)
        elif inspect.isclass(obj):
            return PickleParser.class_to_dict(obj)
        elif PickleParser.is_cls_instance(obj):
            return PickleParser.instance_to_dict(obj)
        elif isinstance(obj, types.CodeType):
            return PickleParser.code_to_dict(obj)
        else:
            raise TypeError(obj)
    ...
    def parse(obj):
        ...
        if isinstance(obj, list):
            res = []
            for i in obj:
                res.append(PickleParser.parse(i))
            return res
        elif isinstance(obj, dict):
            if "--function_type--" in obj and len(obj.keys()) == 1:
                return PickleParser.dict_to_func(obj["--function_type--"])
            if "--class_type--" in obj and len(obj.keys()) == 1:
                return PickleParser.dict_to_class(obj["--class_type--"])
            if "--static_method_type--" in obj and len(obj.keys()) == 1:
                return staticmethod(
                    PickleParser.dict_to_func(obj["--static_method_type--"]["--function_type--"])
                )
            if "--class_method_type--" in obj and len(obj.keys()) == 1:
                return classmethod(PickleParser.dict_to_func(obj["--class_method_type--"]["--function_type--"]))
            if "--module_type--" in obj and len(obj.keys()) == 1:
                return PickleParser.dict_to_module(obj["--module_type--"])
            if "--code_type--" in obj and len(obj.keys()) == 1:
                return PickleParser.dict_to_code(obj["--code_type--"])
            res = {}
            for i in obj:
                res[i] = PickleParser.parse(obj[i])
            return res
        else:
            raise TypeError()
    ...
    def dumps(obj):
        global f_found
        f_found = {}
        return __import__("pickle").dumps(PickleParser.convert(obj))

    @staticmethod
    def loads(obj):
        cur = __import__("pickle").loads(obj)
        return PickleParser.parse(cur)

    @staticmethod
    def dump(obj, fp):
        with open(fp, "wb") as file:
            file.write(PickleParser.dumps(obj))
    ...
    def load(fp):
        try:
            ...
            return PickleParser.loads(data)